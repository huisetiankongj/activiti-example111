...
//            window.console.log("getNodeByKey(" + key + "): ", node.key);
	...
	getSelectedAllParentNodes: function(stopOnParents) {
		var nodeList = [];
		this.rootNode.visit(function(node){
...
//			userEvent = !!ctx.originalEvent,
		...
		if(isActive === flag){
			// Nothing to do
			//TODO 当已经选中状态，再次点击就设置不选中
			//tree.nodeSetActive(ctx, false);
...
$.extend($.ui.fancytree,
	...
	debugLevel: 0,  // used by $.ui.fancytree.debug() and as default for tree.options.debugLevel
 ...
 * 	@private
 *  @method buildEvent
 *  @param {Object} ctx fancytree对象
 *  @param {Object} button 按钮对象  
 *  @param {String} eventType 按钮对象  
 */
function buildEvent(ctx,button,eventType){
	return function(){ 
		button.onClick.call(ctx,{eventType: eventType ,data:button,target:this});
	};
};
 ...
 * 	生成每列的数据
 * 	@private
 *  @method nodeRenderColumns
 *  @param {Object} ctx fancytree对象 
 */
function nodeRenderColumns(ctx){	
	var tree = ctx.tree;
	var node =  ctx.node;
	var data = node.data;
	var permission = ctx.options.permission||[];
	
	var opts = ctx.options.table;
	for(var i in opts.columns){
		i *= 1;
		var primaryIndex = opts.nodeColumnIdx;
		var $td = $(node.tr).find(">td").eq(i);
		var col = opts.columns[i];
		$td.width(col.width).addClass(col.className);
		if(i != primaryIndex  &&  i !=opts.checkboxColumnIdx){
			$td.html(data[col.key]);
		}
		
		var rendText = col.fnRender ? col.fnRender(data[col.key],$td,data,col,permission) : null;
		rendText!=null  && $td.html(rendText);
	}
}
 ...
 * 	生成表格头
 * 	@private
 *  @method nodeRenderHead
 *  @param {JQElement} $table 表格对象
 *  @param {Object} ctx fancytree对象 
 */
function nodeRenderHead($table,ctx){
	var opts = ctx.options.table;
	var colLen = opts.columns.length;
	var headHtml= ["<thead>"];
	
	for(var i in opts.columns){ 
		opts.columns[i].className || (opts.columns[i].className = "");
		headHtml.push("<th class='"+ opts.columns[i].className +"'>");
		headHtml.push(opts.columns[i].title);
		headHtml.push("</th>");
	}
	headHtml.push("</thead>");
	$table.prepend(headHtml.join(""));
	
	var toolHtml = ['<div class="tableButton"><div class="DTTT_container">'];
	for(var i in opts.toolBar){
		var button = opts.toolBar[i];  
		button.id || ( button.id = "" );
		toolHtml.push('<a id="'+button.id+'" style="margin-right: 5px;" class="btn btn-sm ' + button.className + '">' + button.name + '</a>');
	};
	toolHtml.push("</div></div>");
	$table.before(toolHtml.join(""));
	
	for(var i in opts.toolBar){
		var button = opts.toolBar[i];
		if(button.onClick){
			$(".DTTT_container a",$(".tableButton")).eq(i).click(buildEvent(ctx,button,"click"));
		}
	};
}
 ...
 * 	生成用ID和PID结构的数据格式
 * 	@private
 *  @method buildSimpleData
 *  @param {Object} ctx fancytree对象
 *  @param {Object} response 表格数据 
 */
function buildSimpleData(ctx , response){
	if(response.successful){
		ctx.options.permission = response.data.permission||[];
		response = response.data.treeData;
		var tabOptions = ctx.options.table;
		var outData=[];
		if(ctx.options.source.isSimple){//是否记用ID和PID数据结构
			var mapResponse={};
			var ID="id",PID="pid";
			if(ctx.options.source.key){
				ID = ctx.options.source.key.id ? ctx.options.source.key.id : ID;
				PID = ctx.options.source.key.pid ? ctx.options.source.key.pid : PID;
			}
			for(var i in response){
				var rowData = mapResponse[response[i][ID]] = response[i];
				var primaryIndex =   tabOptions.nodeColumnIdx;
				var key = tabOptions.columns[primaryIndex].key;
				rowData.title =	rowData[key];
			...
			for(i in response){
				...
				} else if(mapResponse[pid] == null){//要有一定的容错性，可能没有的ID是没有PID的，这样要把他当成根节点
					mapResponse[id].folder=true;
					outData.push(rowData);
				}else{ 
					mapResponse[pid].folder=true;
					if(mapResponse[pid].children){
					   	mapResponse[pid].children.push(rowData);
					} else{
						mapResponse[pid].children = [rowData];
					}
				}
			}
			return outData;	
...
$.ui.fancytree.registerExtension("table", {
	...
	options: {
		indentation: 22,        // indent every node level by 16px
	...
	treeInit: function(ctx){
		...
		tree.columnCount = tree.options.table.columns.length;
		$(tree.tbody).empty();
...
//		tree._triggerNodeEvent("renderNode", ctx);
	...
	nodeLoadChildren: function(ctx, source) {
		...
		if(source.url){
			// `source` is an Ajax options object
			ajax = $.extend({}, ctx.options.ajax, source);
			ajax.data && (ajax.data = JSON.stringify(ajax.data));
			if(ajax.debugDelay){
				// simulate a slow server
				delay = ajax.debugDelay;
				if($.isArray(delay)){ // random delay range [min..max]
			...
			/*
			if(ajax.refBy){
				if((typeof ajax.refBy) === 'string')
					source.setRequestHeader('functionReferBy', ajax.refBy);
				else
					source.setRequestHeader('functionReferBy', JSON.stringify(ajax.refBy));
			}*/
			//source.setRequestHeader('functionReferBy', "limingmin2312312");
			// TODO: change 'pipe' to 'then' for jQuery 1.8
