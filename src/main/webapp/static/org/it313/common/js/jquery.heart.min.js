(function(w) {
	...
	var callback = function(resp, fnCallback, failure) {
		if (resp.successful) {
			...
			layer.alert('您当前还未登录，点击确定后重新登录！',function(){
				location.href = rootPath + "/front/login.html?returnUrl="+encodeURIComponent(window.location.href);
			});
	...
	var call = function(ajaxType, _url, _param, _success, method, refBy, async, failure, isShowTips) {
		...
		setting.beforeSend = function(xhr) {
			...
			if ((isShowTips == null) || (isShowTips == undefined) || (isShowTips === true)) {
				currentLoadIndex = layer.load();
				loadIndexMap[currentLoadIndex] = currentLoadIndex;
			}
		};
		...
		setting.complete = function(XMLHttpRequest, textStatus) {
			// 判断是否需要显示遮罩，未传值或者true->显示，false和不正确参数->不显示
			if ((isShowTips == null) || (isShowTips == undefined) || (isShowTips === true)) {
				delete loadIndexMap[currentLoadIndex];
				$.isEmptyObject(loadIndexMap) && layer.closeAll('loading');
			}
		};
