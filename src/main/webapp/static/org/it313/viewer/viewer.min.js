...
ImgViewer={};
ImgViewer.showImg=function(config){
		function initHtml(aData){
			...
			$.each(aData,function(){
				...
				ele += "<li><img data-original='"+src+"' src='"+src+"'/></li> ";
			})
			$("body").append(["<ul id='element_id' style='display:none'>",
			                  ele,
			                  "</ul>"
			                  ].join(" "));
			var viewer = new Viewer(document.getElementById('element_id'));
			$('#element_id li img[data-original="'+config.activeImage+'"]').trigger('click');
		};
